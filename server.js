...
export function queueNotification(sellerId, event, data) {
  ...
  pendingNotifications.get(id).push({
    event,
    data,
    ts: Date.now(),
  })
...
io.on("connection", (socket) => {
  ...
  socket.on("register_seller", (sellerId) => {
    ...
    sellerSockets.forEach((sockets, sid) => {
      if (sockets.has(socket.id)) {
        ...
        if (sockets.size === 0) {
          sellerSockets.delete(sid)
        }
      }
    })

    if (!sellerSockets.has(id)) {
      sellerSockets.set(id, new Set())
    }
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (id && sellerSockets.has(id)) {
      ...
      if (sellerSockets.get(id).size === 0) {
        sellerSockets.delete(id)
      }
